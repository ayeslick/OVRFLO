...
**What to build:** Deploy and seed still run "deploy vault, then `registerOvrflo(vault)`." The runbook reads creation wiring and minter bindings. The deployment artifact gains `border` under the same paired-optional consume rule as vault and lending. Fixture deploy helpers return the border as a new tuple member. `DeploySize` gates `OVRFLOBorder`. The client env contract gains no border variable; the web learns `border` from factory discovery in ticket 07.
...
Do not edit the plan. Do not start other units. Do not add NEXT_PUBLIC_OVRFLO_BORDER.
...
when the border member is added. Seed smoke: if MAINNET_RPC_URL is unavailable,
...
- [ ] Seed writes `border` into `deployments/local.json`
- [ ] Artifact `border` follows the paired-optional consume rule (both present or both derived)
- [ ] Fixture return tuple includes `border`; old positional destructurers fail to compile until updated
- [ ] `test/DeploySize.t.sol` gates `OVRFLOBorder` against EIP-170 and EIP-3860 caps
- [ ] Client env contract is unchanged; `NEXT_PUBLIC_OVRFLO_BORDER` is obsolete, not added
