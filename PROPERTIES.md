...
- [x] **GL-86** Direct underlying transfer to vault does not inflate wrappedUnderlying. VALID_STATE, EXPLORATORY, LOW. Evidence: wrappedUnderlying is an internal mapping, not balance-derived; direct underlying transfer cannot increase it. Sources: SPEC-02. Distinct from GL-02/GL-59. **Covered by fizz_donate handler (F3, review 2026-07-18)**.
...
- [x] **SP-17** deposit fee floored (observed from actor underlying delta, not recomputed formula). VALID_STATE, SHOULD-HOLD, MEDIUM. Sources: RD-14. Called after: deposit.
...
- [x] **SP-19** lending fee floored (observed from treasury balance delta, not recomputed formula). VALID_STATE, SHOULD-HOLD, MEDIUM. Sources: RD-16. Called after: sellStreamToLiquidity, buyListing, createBorrowerLoanPool.
