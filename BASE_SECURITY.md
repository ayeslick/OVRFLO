...
## Never Miss the Basics

A per-function pre-flight checklist. Run these five questions on every external/public function before diving into the deep vulnerability catalogs below. The questions are deliberately basic — most real exploits fail one of these, not an obscure pattern.

1. **Who can call this function?** Check every modifier, not just the obvious `onlyOwner`. Inherited modifiers, implicit "anyone" access, and role-based gating all count. If you cannot write down the caller set in one sentence, you do not know it.
2. **What state changes before an external call?** Reentrancy. Trace every external call (`safeTransfer`, `createWithDurations`, callback invocations) and confirm state is written before, not after. CEI is the default; deviations need a reason.
3. **What happens with zero as input?** Edge cases. Zero amount, zero address, empty array, missing ID. Many exploits are not exotic — they are the zero case the author assumed could not happen.
4. **Can this be called twice?** Replay. Same caller, same arguments, same tx or a later one. If the function should be one-shot, what marks it consumed?
5. **What does the function NOT check?** Missing validation. This is the inverse of the audit checklist — instead of confirming what is checked, list what is not. The unchecked input is the bug.

**Inheritance attack surface.** Every time you inherit a base contract (OpenZeppelin `ERC20`, `Ownable2Step`, `ReentrancyGuard`, etc.), list every external/public function you did **not** override. That list is your unguarded attack surface — inherited functions whose access control, input validation, and state transitions you did not author and may not have audited. Apply the five questions above to each one.
