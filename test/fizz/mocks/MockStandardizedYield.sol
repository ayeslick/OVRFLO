...
contract MockStandardizedYield is IStandardizedYield {
    ...
    function yieldToken() external view returns (address) {
        return yieldTokenAddr;
    }

    // Minimal stubs for interface compliance
    function deposit(address, address, uint256, uint256) external payable returns (uint256) {
        return 0;
    }
    ...
    function claimRewards(address) external pure returns (uint256[] memory) {
    ...
    function getTokensIn() external pure returns (address[] memory) {
        return new address[](0);
    }

    function getTokensOut() external pure returns (address[] memory) {
        return new address[](0);
    }
    ...
    function previewDeposit(address, uint256) external pure returns (uint256) {
        return 0;
    }

    function previewRedeem(address, uint256) external pure returns (uint256) {
        return 0;
    }

    function assetInfo() external pure returns (AssetType, address, uint8) {
        return (AssetType.TOKEN, address(0), 18);
    }
