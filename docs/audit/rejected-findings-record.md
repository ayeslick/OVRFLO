...
- **Disproof:** The bound stream lockup is OVRFLO Streams — a fork of Sablier v2-core v1.1.2 (Solidity `SablierV2LockupLinear`, identity `OVRFLOStream`). Vault getter `sablierLL` **no longer** resolves to canonical `0xAFb979d9afAd1aD27C5eFf4E27226E3AB9e5dCC9`. Upstream v1.1 (and the fork, by **plan R3**) requires `msg.sender` to be the stream **sender**, **NFT owner**, or **approved operator** — otherwise `SablierV2Lockup_Unauthorized`. There is **no permissionless withdraw**. When the book holds the NFT it **is** the recipient; only the book (or its approved operators) can withdraw. The OVRFLO vault as sender may only withdraw `to == recipient` (the book), which is trusted.
- **Evidence:** ACL table in `sablier-interface-contract.md` (keyed to the fork; provenance fork of v1.1.2; ACL preserved per R3). Historical verification also used `sablier-labs/v2-core` tag `v1.1` and the former canonical bytecode at `0xAFb979d9afAd1aD27C5eFf4E27226E3AB9e5dCC9` — that address is no longer the bound contract.
- **Why this matters:** The v1.1-vs-later-version distinction is the whole finding. Some newer Sablier Lockup docs describe a public "withdraw to recipient" path; that does **not** apply to the v1.1 lineage the fork preserves. An auditor who checks only canonical Sablier and reaches the right answer has done so by luck, not from evidence. Cite R3 and the fork binding.
...
- **Disproof (security):** Self-lending is economically irrational — the user posts `capacity` in underlying, borrows against their own stream, receives `borrowAmount - fee` back, and the fee goes to treasury. Nobody does this intentionally. The loan is still fully resolvable via `claimPoolShare()` and `closeLoan()` (the book calls `sablier.withdraw(streamId, address(this), amount)` via `_claimFair` harvest, which has no self-transfer issue). No fund safety issue: no one else is affected, accounting is correct, nothing is permanently locked (`closeLoan` is permissionless and works at maturity). The only impact is a self-inflicted UX edge case for irrational behavior.
...
| Loan accounting underflow | `drawn + repaid <= obligation` guaranteed: `claimPoolShare` (via `_claimFair`) harvests at most `_outstanding(loan)` from the stream, `repayLoan` requires `amount <= outstanding` (G-24); no sequence of claim/repay/close can overflow `drawn + repaid` past `obligation` |
...
7. **Self-loan `repayLoan` revert?** — UX edge case, not a security finding. Self-lending is economically irrational. Loan still resolvable via `claimPoolShare`/`closeLoan`. Self-match prevention guards added as a correctness fix (2026-06-28); see rejected finding above and pattern #4 in `ovrflo-critical-patterns.md`.
...
### CR-I2 — Hardcoded Sablier address in OVRFLO — SUPERSEDED by constructor bind + factory admission

- **Original claim (Informational):** `sablierLL` is hardcoded to a mainnet address, limiting portability to other chains.
- **Historical rejection:** Hardcoding was intentional transparency against canonical Sablier V2 at `0xAFb979d9afAd1aD27C5eFf4E27226E3AB9e5dCC9`.
- **Current binding (post KTD6 / R8):** The vault stream address is a constructor argument (getter stays `sablierLL()`). `factory.ovrfloStream()` is the one-shot admitted lockup. `registerOvrflo` / `registerLending` require the candidate binds that address (`StreamNotCanonical` otherwise). Matching audited vault bytecode alone is not a safe stream-binding predicate. `SablierMismatch` still proves vault and lending bind the same stream. The Safe still checks creation bytecode off-chain.
- **Evidence:** `src/OVRFLO.sol` constructor `stream` arg; `src/OVRFLOFactory.sol` `setOvrfloStream` / registration checks; `docs/audit/sablier-interface-contract.md` R8 trust-shift section.
...
- **Disproof:** The bound lockup is OVRFLO Streams (fork of Sablier v2-core v1.1.2). Vault `sablierLL` **no longer** resolves to `0xAFb979d9afAd1aD27C5eFf4E27226E3AB9e5dCC9`. Upstream v1.1 withdraw ACL — preserved on the fork by **plan R3** — reverts `SablierV2Lockup_Unauthorized` unless `msg.sender` is the stream sender, the NFT owner, or an approved operator. `src/OVRFLO.sol` has no `sablier.withdraw` code path, so the vault-as-sender cannot push one; `src/OVRFLOLending.sol` never calls `approve` or `setApprovalForAll` on the stream NFT, so no operator exists. The divergence the finding describes cannot be produced.
- **Evidence:** ACL table in `sablier-interface-contract.md` (fork binding; R3 ACL preservation). Historical pin was `sablier-labs/v2-core` tag `v1.1` / former canonical `0xAFb979…`. Reproducible against the bound lockup by `test/fork/OVRFLOLendingMainnetFork.t.sol` — including the `to = address(lending)` case, which discriminates v1.1 from the later permissionless-`to == recipient` ACL.
