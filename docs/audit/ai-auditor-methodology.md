...
| `evm-audit-defi-lending` | Self-repaying loans, no health check, no liquidation, `createBorrowPool`/`createLenderPool`/`repayLoan`/`closeLoan` lifecycle, `outstanding` relation | I-10, E-2; G-17, G-18; `closeLoan()`, `poolClaimLoan()`, `repayLoan()` |
...
| `evm-audit-erc721` | Sablier stream NFT custody, `transferFrom` escrow, `requireEligible` checks, NFT ownership through loan lifecycle | X-5, X-2; `sellIntoOffer()`, `postSaleListing()`, `buyListing()`, `createBorrowPool()`, `closeLoan()` |
