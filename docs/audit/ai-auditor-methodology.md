...
| **CROPS** — Censorship Resistance, Open Source, Privacy, Security | **Censorship resistance**: on-chain Safe timelock gates all admin actions; the multisig can cap a market's deposits by setting a positive deposit limit (0 = unlimited) or block new lending by setting `feeBps = 100%` (emergency circuit breaker), but cannot pause claims, wraps, or unwraps. **Open source**: yes (MIT, verified). **Privacy**: all flows are public onchain. **Security**: audited (point-in-time, not ongoing). | What can the multisig do after the timelock delay that harms users? Can they front-run a user by changing config mid-transaction? What happens if the multisig disappears — do any liveness-critical paths break? |
...
| **Access control** | Multisig → `OVRFLOFactory` → `OVRFLO` vault (`onlyAdmin`, G-1). `OVRFLOLending` has `onlyOwner`. Two-step ownership on factory and lending. On-chain Safe timelock on all operational setters. `setLendingFee(100%)` and `setMarketDepositLimit(low)` serve as emergency circuit breakers. | Enforced via ownership chain + timelock. But `setMarketDepositLimit` can lower below current deposits (I-6) and `setAprBounds` does not revalidate existing orders (I-4, I-5). Probe: what can the multisig do after the timelock delay that harms users? | I-6, I-4, I-5, G-1 |
...
| `evm-audit-access-control` | Multisig → factory → vault chain, `onlyAdmin`/`onlyOwner`/`onlyOwner` gating, two-step ownership, on-chain Safe timelock, `setLendingFee(100%)` as circuit breaker, `setMarketDepositLimit` lowering, `setAprBounds` non-retroactivity | I-6, I-4, I-5, I-9; G-1, G-2, G-3; admin entry points |
...
| `evm-audit-governance` | Multisig + two-step ownership + on-chain Safe timelock, `setLendingFee(100%)` / `setMarketDepositLimit(low)` as emergency circuit breakers, admin key compromise, no vault admin migration path | I-6, I-4, I-5; `transferOwnership()`, `acceptOwnership()` |
