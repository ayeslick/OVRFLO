...
**Behavioral pins.** `repay` with `amount > outstanding` reverts (as today). `claim` caps `amount` at claimable internally; callers pass `type(uint128).max` to claim everything. `supply` requires exact UNIT multiples; `borrow` targets floor to UNIT. `tickDepths(market)` iterates spacing-multiples within `[aprMin, aprMax]` as read at call time, summing `root − filled` across live epochs per tick. `loansOf(positionId, startSeq, maxN) → (entries[], nextSeq)` where each entry is `(loanId, contribution, claimable)`; `maxN > 0`; `nextSeq = 0` means exhausted. `advanceEpochCursor(market, aprBps, maxSteps)` requires `maxSteps > 0`, is a no-op success when nothing qualifies, advances while the epoch's available depth is `< MIN_LIQUIDITY_AMOUNT` and the cursor is below `currentEpoch`, and emits `EpochCursorAdvanced` only when the cursor actually moves (a no-op emits nothing; a `borrow` that advances the cursor is silent — `Borrowed.epoch` is its checkpoint).
...
*Recorded 2026-08-08 (U5, coordinator-implemented; amended same day by the U5 review):* (1) `PositionMissing` minted for the position-side named views (`positionState`, `loansOf`) — mirrors `LoanMissing`; the closed catalog carried no position-view error. (2) **Reversed by review:** `loansOf` with `maxN == 0` reverts `ZeroSteps`, not `ZeroAmount` — the catalog already held the exact-fit selector (a zero iteration bound on a bounded scan), and the review correctly flagged the coordinator's asymmetry in minting for its own unit while stretching elsewhere. (3) `tickDepths`/`tickState` on a market with unset spacing revert `SpacingUnset`; **and (6th, recorded post-review):** `tickState` deliberately validates spacing ONLY — not tick alignment or APR bounds — so positions at ticks outside the owner-mutable bounds stay readable through the named view. (4) A `borrow` that advances the cursor does so silently — `Borrowed.epoch` is the checkpoint; `EpochCursorAdvanced` is reserved for the recovery valve (now test-asserted via `vm.recordLogs`). (5) The rollover predicate is terminal-capacity only: `height == MAX_HEIGHT && atCapacity()` — review upgraded this from judgment call to FORCED (bare `atCapacity` deletes R4's growth). *Security notes from the same review:* the ladder view is O(rungs × epochs) with no pagination — spacing sanity ((aprMax−aprMin)/spacing bounded, e.g. ≤ ~400 rungs) is a multisig onboarding-checklist item since spacing is set-once; narrowing APR bounds hides (never locks) out-of-window liquidity from the ladder — frontends enumerate a lender's own positions via `lenderPositionAt`, not the ladder; `loansOf.startSeq` must be a `nextSeq` from the same position (arbitrary values silently truncate — discovery-only surface); the terminal-capacity TRUE branch is exercisable only via the harness override (8^7 real appends is prohibitive) — its false branch is covered by the real 4096-leaf growth run.
...
| `EpochBacklog`, `ZeroSteps` | `borrow` (cap exceeded — error text directs to `advanceEpochCursor`); `advanceEpochCursor`; `ZeroSteps` also `loansOf` (`maxN == 0` — reversed from `ZeroAmount` by the 2026-08-08 U5 review: same zero-iteration-bound semantics) |
