...
#### 2026-09-02 owner deferral: CS1-U6

Recorded after tickets 01 and 02 resolved. CS1-U6 (fuzz and invariant re-derivation, ticket 06) is deferred until the owner records start-OK on that ticket; it runs after the feature tickets. Sweep rule 4's "run the fizz-sync path after U2–U4 land" is postponed with it. Until 06 starts, every other unit makes only the minimum edit that keeps plain `forge test` green in the fuzz, invariant, and `test/fizz/` Foundry files it breaks, logs that edit on its ticket, and does not run `FOUNDRY_PROFILE=invariant` or the fizz harness. CS1-U8 (docs) no longer depends on CS1-U6; onboarding §5 describes the KD13 invariants as pinned, not as re-derived. The Verification Contract items that name invariant runs (item 2, item 7 *Column solvency* and *Wrap reserve*) are satisfied by 06 when it runs, not by the tickets before it.
