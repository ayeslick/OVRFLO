...
- **Column solvency (replaces the combined check in `docs/agents/onboarding.md` §5):** `ovrfloToken.totalSupply() <= ptToken.balanceOf(vault) + underlying.balanceOf(border)`. Per-origin equality also holds: `totalSupply == Σ marketTotalDeposited + border.wrappedUnderlying`. The fizz property `property_vault_combined_solvency` (GL-07) is rewritten against vault+border.
