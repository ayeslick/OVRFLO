...
- Post-time fill-or-rest algorithm (owner pin 2026-09-01). `post` must not wrap core `borrow` in `try/catch`: `borrow` reverts for ineligible streams, invalid ticks, and empty depth with different errors, and a blanket catch would let an ineligible stream rest. The pinned order is:
