...
- [`wagmi-query-key-dedup-makes-cross-component-hook-duplication-free.md`](wagmi-query-key-dedup-makes-cross-component-hook-duplication-free.md) — a third instance of the same meta-lesson, this time about *not* consolidating: two components independently calling the same read hook with matching args already dedupe via TanStack Query's cache key, so the "duplication" a review pass flags there is often not real. Found via the 2026-07-27 `/improve-codebase-architecture` pass over `web/*`.
